...
%% Adjust layout
if isfield(options, 'stage_name') && ~isempty(options.stage_name)
    sgtitle(sprintf('Prestressed Concrete Beam Analysis — %s', options.stage_name), ...
...
% Sign convention note +M = Hogging (tension at top)
text(0.02, 0.98, '+M = Hogging (tension at bottom)', ...
    'Units', 'normalized', 'VerticalAlignment', 'top', ...
    'FontSize', 8, 'Color', [0.5, 0.5, 0.5]);
text(0.02, 0.93, '-M = Sagging (tension at top)', ...
    'Units', 'normalized', 'VerticalAlignment', 'top', ...
    'FontSize', 8, 'Color', [0.5, 0.5, 0.5]);
