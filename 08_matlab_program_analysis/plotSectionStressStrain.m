...
if ~isfield(options, 'stage_label'),    options.stage_label = ''; end
...
if strcmp(options.plot_type, 'both')
    if ~isempty(options.stage_label)
        sgtitle(sprintf('[%s]  Stress & Strain at x = %.1f in (%.1f%% of span)', ...
            options.stage_label, x_actual, x_actual/L*100), ...
