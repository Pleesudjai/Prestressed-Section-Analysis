...
beam.num_segments = 1000;          % Number of segments for analysis
beam.x = linspace(0, beam.L, beam.num_segments + 1);  % x-coordinates along beam

% Cross-section plot locations (fractions of span, 0–1)
% Edit this list to change which sections are plotted and saved.
beam.x_plot_fractions = [0, 120, 240, 384] / beam.L;  % 0 in, 120 in, 240 in, 384 in
...
% At TRANSFER (using f'ci = 4.8 ksi):
%   Compression (general):  +0.60 * f'ci = +2.88 ksi
%   Compression (ends):     +0.70 * f'ci = +3.36 ksi
%   Tension (general):      -3*sqrt(f'ci_psi)/1000 ksi
%   Tension (ends):         -6*sqrt(f'ci_psi)/1000 ksi
materials.f_ci_allow    =  0.60 * materials.fci;                        % +2.88 ksi
materials.f_ci_allow_end =  0.70 * materials.fci;                       % +3.36 ksi
materials.f_ti_allow    = -3.0 * sqrt(materials.fci * 1000) / 1000;    % -0.208 ksi
materials.f_ti_allow_end = -6.0 * sqrt(materials.fci * 1000) / 1000;   % -0.416 ksi

% At SERVICE (using f'c = 6.0 ksi, Class C member):
%   Compression (sustained loads SW+SDL): +0.45 * f'c = +2.70 ksi
%   Compression (total loads SW+SDL+LL):  +0.60 * f'c = +3.60 ksi
%   Tension Class C:  -12*sqrt(f'c_psi)/1000 ksi = -0.929 ksi
materials.f_cs_allow_sust  =  0.45 * materials.fc;                      % +2.70 ksi
materials.f_cs_allow_total =  0.60 * materials.fc;                      % +3.60 ksi
materials.f_ts_allow       = -12.0 * sqrt(materials.fc * 1000) / 1000; % -0.929 ksi (Class C)
...
% --- Live Load: 420 psf over 10 ft flange width ---
%   w_LL = 420 * (120/12) / 12 / 1000 = 0.350 kip/in (4.20 kip/ft)
w_LL = 420 * (120/12) / 12 / 1000;     % kip/in
