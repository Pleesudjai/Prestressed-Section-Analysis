...
if ~isempty(prestress) && isfield(prestress, 'tendons')
    ...
    if ~isempty(options.beam)
        ...
        for i = 1:length(prestress.tendons)
            ...
            if isfield(tendon, 'x_position') && ~isempty(tendon.x_position)
                x_tendon_x = tendon.x_position;
            else
                x_tendon_x = section.xc;  % default to section centroid x
            end

            % Store position for dimensioning
            tendon_positions = [tendon_positions; y_tendon, x_tendon_x, tendon.Aps, i];

            theta = linspace(0, 2*pi, 30);
            x_circle = x_tendon_x + (d_tendon/2) * cos(theta);
